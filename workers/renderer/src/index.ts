...
// Same counter as the static Next.js site's src/components/YandexMetrika.tsx.
// Dynamic pages strip ALL inline template scripts (see the HTMLRewriter script
// handler below) to avoid Next.js re-hydrating over injected content, which
// silently dropped Metrika from every factory-published article. Re-inject it
// explicitly here so dynamic pages (the majority of traffic) are measured too.
