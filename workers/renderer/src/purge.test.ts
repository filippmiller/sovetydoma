...
function seedCategoryRows(store: Map<string, Response>, category: string, dynamic: number, stat: number) {
  ...
  store.set(
    `https://1001sovet.ru/__internal/cat-rows/${category}?render=14`,
...
describe('POST /__purge', () => {
  ...
  it('purges article + cat-rows + hub + sitemap keys when slug is given', async () => {
    ...
    try {
      ...
      const expected = [
        'https://1001sovet.ru/ekonomiya/kak-sekonomit-na-vode/?render=14',
        'https://1001sovet.ru/__internal/cat-rows/ekonomiya?render=14',
        'https://1001sovet.ru/stati//1?render=14',
        'https://1001sovet.ru/stati/ekonomiya/1?render=14',
        'https://1001sovet.ru/stati/ekonomiya/2?render=14',
        'https://1001sovet.ru/stati/ekonomiya/3?render=14',
      ...
      assert.equal(cache.store.has('https://1001sovet.ru/__internal/cat-rows/ekonomiya?render=14'), false)
    } finally {
      cache.restore()
    }
  })
  ...
  it('category-only purge skips the article key and deletes only page 1..2 of a small hub', async () => {
    ...
    try {
      ...
      const expected = [
        'https://1001sovet.ru/__internal/cat-rows/kulinaria?render=14',
        'https://1001sovet.ru/stati//1?render=14',
        'https://1001sovet.ru/stati/kulinaria/1?render=14',
        'https://1001sovet.ru/stati/kulinaria/2?render=14',
        'https://1001sovet.ru/sitemap-dynamic.xml?generator=v3',
        'https://1001sovet.ru/zen.xml?generator=v4',
      ]
      assert.deepEqual(sorted(cache.deleted), sorted(expected))
